...
function M.set_ordered_value(line, val)
	local function digitfunc()
		return (line:gsub("^(%s*)%d+", "%1" .. val, 1))
	end
...
-- trim whitespace
function M.get_whitespace_trimmed(str)
	return str:gsub("%s*$", "", 1)
end

-- delete % signs
function M.get_percent_filtered(pat)
	return pat:gsub("%%", "")
end
...
function M.get_list_start(cur_linenum, list_types)
	...
	if list_types then
		while
			(M.is_list(line, list_types) and M.get_indent_lvl(line) >= cur_indent)
